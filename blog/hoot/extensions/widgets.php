		...
		?><div class="hoot-widget-field hoot-widget-field-type-<?php echo ( strlen( $type ) < 15 ) ? sanitize_html_class( $type ) : 'custom' ?> hoot-widget-field-<?php echo sanitize_html_class( $id ) ?>"><?php
			...
			switch( $type ) {
				...
				case 'icon':
					...
					<div id="<?php echo $this->hoot_get_field_id( $id, $repeater ) . '-icon-picked' ?>" class="hoot-of-icon-picked"><i class="<?php echo esc_attr( $iconvalue ) ?>"></i><span><?php _e( 'Select Icon', 'responsive-brix' ) ?></span></div>
					<div class="clear"></div>
					<div id="<?php echo $this->hoot_get_field_id( $id, $repeater ) . '-icon-picker-box' ?>" class="hoot-of-icon-picker-box">
						<div class="hoot-of-icon-picker-list"><i class="fas fa-ban hoot-of-icon-none" data-value="0" data-category=""><span><?php _e( 'Remove Icon', 'responsive-brix' ) ?></span></i></div>
						<?php
				...
				case 'group':
					...
					?><div class="hoot-widget-field-group" data-id="<?php echo esc_attr( $id ) ?>">
						...
						?>
						<div id="add-<?php echo rand(1000, 9999); ?>" class="hoot-widget-field-group-add<?php if ( !empty($maxlimitcheck) && $groupcount >= $maxlimitcheck ) echo ' maxreached'; ?>" data-iterator="<?php echo is_array( $value ) ? max( array_keys( $value ) ) : 0; ?>" <?php echo $maxlimit.$limitmsg ?>><?php _e('Add', 'responsive-brix') ?></div>
					</div>
					<?php
					break;
				...
				case 'collapse':
					...
					?><div class="hoot-widget-field-collapse" data-id="<?php echo esc_attr( $id ) ?>">
						<div class="hoot-collapse-head"><h3><?php if ( !empty( $name ) ) echo esc_html($name); else _e('Group', 'responsive-brix') ?> <i class="fas fa-sort"></i></h3></div>
